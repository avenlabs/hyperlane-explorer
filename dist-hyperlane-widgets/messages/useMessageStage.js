...
export function useMessageStage({ message, multiProvider, explorerApiUrl = HYPERLANE_EXPLORER_API_URL, retryInterval = 2000, }) {
    ...
    const fetcher = useCallback(() => {
        ...
        if (message.status === MessageStatus.Failing && data)
            return;
...
async function getFinalityBlocks(domainId, multiProvider) {
    const metadata = await multiProvider.getChainMetadata(domainId);
    if (metadata?.blocks?.confirmations)
        return metadata.blocks.confirmations;
    else
        return DEFAULT_FINALITY_BLOCKS;
}
async function getBlockTimeEst(domainId, multiProvider) {
    const metadata = await multiProvider.getChainMetadata(domainId);
    return metadata?.blocks?.estimateBlockTime || DEFAULT_BLOCK_TIME_EST;
}
async function tryFetchChainLatestBlock(domainId, multiProvider) {
...
async function tryFetchLatestNonce(domainId, multiProvider, explorerApiUrl) {
    const metadata = multiProvider.tryGetChainMetadata(domainId);
    if (!metadata)
        return null;
    logger.debug(`Attempting to fetch nonce for:`, metadata.name);
    try {
        const response = await fetchWithTimeout(`${explorerApiUrl}/latest-nonce`, {
            method: 'POST',
            headers: {
